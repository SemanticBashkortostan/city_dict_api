...
#      2) Add fetching lat and lon from .osm file
class OsmFetcher

  ISHIMBAY_BOUNDING_BOX = {:top => 53.4731, :left => 55.9502,
    :bottom => 53.3999, :right => 56.1322}
  SALAVAT_BOUNDING_BOX = {:top => 53.3862, :left => 55.8489,
    :bottom => 53.3054, :right => 55.999}
  STERLITAMAK_BOUNDING_BOX = {:top => 53.7523, :left => 55.7659,
    :bottom => 53.5619, :right => 56.1744}
  NEFTEKAMSK_BOUNDING_BOX = {:top => 56.1428, :left => 54.1406,
    :bottom => 55.9649, :right => 54.4221}
  UFA_BOUNDING_BOX = {:top => 54.8943, :left => 55.7638,
    :bottom => 54.5258, :right => 56.21}
  ...
  def get_part_of_map(main_map = "#{Rails.root}/project_files/osm_maps/bashkortostan.osm")
    ...
    exec = "osmosis --read-xml file=\"#{main_map}\" --bounding-box \
      top=#{@bounding_box[:top]} left=#{@bounding_box[:left]} \
      bottom=#{@bounding_box[:bottom]} right=#{@bounding_box[:right]} \
      --write-xml file=\"#{@filename}\""
    system exec
  end
  ...
  #TODO: Handle updating of the .osm file!!!
  def self.fill_db!
    osm_arr = [
    ...
    normalized_city_names = City.pluck(:name).map do |name|
      VocabularyEntry.get_normalized_name(name)
    end
    normalized_city_names_set = normalized_city_names.to_set

    osm_arr.each do |(city_eng_name, filename, bounding_box)|
      osm = OsmFetcher.new bounding_box, filename
      osm.get_features.each do |name, amenity, osm_id|
        normalized_name = VocabularyEntry.get_normalized_name(name)
        next if normalized_city_names_set.include?(normalized_name)

        token = VocabularyEntry.find_or_create_by_name_or_normalized_name(name)

        city_id = City.find_by_eng_name(city_eng_name).id
        type = amenity
        params = {city_id: city_id, source: :osm, type_name: type, vocabulary_entry_id: token.id}
